/**
 * Serverless GET/PUT/DELETE /api/assignments/[id].
 */
...
export async function GET(
  ...
  try {
    ...
    console.error('GET /api/assignments/[id] error:', e);
...
export async function PUT(
  ...
  try {
    ...
    await assignmentsCollection().doc(id).update(update);
    const updated = await assignmentsCollection().doc(id).get();
    return NextResponse.json(docToJson(updated));
  } catch (e) {
    console.error('PUT /api/assignments/[id] error:', e);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
