/**
 * Serverless GET/PUT/DELETE /api/grade-components/[id].
 */
...
export async function GET(
  ...
  try {
    ...
    const doc = await gradeComponentsCollection().doc(id).get();
    ...
    console.error('GET /api/grade-components/[id] error:', e);
...
export async function PUT(
  ...
  try {
    ...
    await gradeComponentsCollection().doc(id).update(update);
    const updated = await gradeComponentsCollection().doc(id).get();
    return NextResponse.json(docToJson(updated));
  } catch (e) {
    console.error('PUT /api/grade-components/[id] error:', e);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await gradeComponentsCollection().doc(id).delete();
    return NextResponse.json({ message: 'Deleted successfully' });
  } catch (e) {
    console.error('DELETE /api/grade-components/[id] error:', e);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
