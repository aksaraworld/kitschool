/**
 * Serverless GET/PUT/DELETE /api/grades/[id].
 */
...
export async function GET(
  ...
  try {
    ...
    const doc = await gradesCollection().doc(id).get();
    ...
    console.error('GET /api/grades/[id] error:', e);
...
export async function PUT(
  ...
  try {
    ...
    await gradesCollection().doc(id).update(update);
    const updated = await gradesCollection().doc(id).get();
    return NextResponse.json(docToJson(updated));
  } catch (e) {
    console.error('PUT /api/grades/[id] error:', e);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await gradesCollection().doc(id).delete();
    return NextResponse.json({ message: 'Deleted successfully' });
  } catch (e) {
    console.error('DELETE /api/grades/[id] error:', e);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
