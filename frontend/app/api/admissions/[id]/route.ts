/**
 * Serverless GET/PUT/DELETE /api/admissions/[id].
 */
...
export async function GET(
  ...
  try {
    ...
    const doc = await admissionsCollection().doc(id).get();
    ...
    console.error('GET /api/admissions/[id] error:', e);
...
export async function PUT(
  ...
  try {
    ...
    await admissionsCollection().doc(id).update(update);
    const updated = await admissionsCollection().doc(id).get();
    return NextResponse.json(docToJson(updated));
  } catch (e) {
    console.error('PUT /api/admissions/[id] error:', e);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await admissionsCollection().doc(id).delete();
    return NextResponse.json({ message: 'Deleted successfully' });
  } catch (e) {
    console.error('DELETE /api/admissions/[id] error:', e);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
