/**
 * Serverless GET/PUT/DELETE /api/submissions/[id].
 */
...
export async function PUT(
  ...
  try {
    ...
    await submissionsCollection().doc(id).update(update);
    const updated = await submissionsCollection().doc(id).get();
    return NextResponse.json(docToJson(updated));
  } catch (e) {
    console.error('PUT /api/submissions/[id] error:', e);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
